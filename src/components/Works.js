import '../styles/Works.css'
// import React, {useState} from "react"


const Works = () => {


    // function bottomLinks(e) {
    //     e.target.style.display = 'block';
    // }
    

    return ( 
        <div className="project">
            <div className="project-item"><img src="/assets/work9.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Ellcrys</h3>
                    <a href="https://www.ellcrys.org/team" className="">Ellcrys </a>
                    {/* <a href="www." */}
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work8.png" alt="projects" />
                <div className="project-content">
                    <a href="https://www.ellcrys.org/team" className="">Orokolabs </a>
                    <a href="https://www.ellcrys.org/team" className="">Github </a>
                    <a href="https://www.ellcrys.org/team" className="">UI Design </a>
                   
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work1.png" alt="projects" />
                <div className="project-content">
                    <a href="https://www.ellcrys.org/team" className="">Urban</a>
                    <a href="https://www.ellcrys.org/team" className="">UX Case Study</a>
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work2.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">GRLD</h3>  
                    <a href="https://www.ellcrys.org/team" className="">UX Case Study </a>
                    {/* <a href="www." */}
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work5.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Sample</h3>
                    <a href="https://www.ellcrys.org/team" className="">UI Design </a>
                    {/* <a href="www." */}
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work6.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Rexaan Logo</h3>
                    <a href="https://www.ellcrys.org/team" className="">Logo Design </a>
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work7.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Checkout Page</h3>
                    <a href="https://www.ellcrys.org/team" className="">UI Design </a>
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work3.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Promo Banner</h3>
                    <a href="https://www.ellcrys.org/team" className="">Promo Design </a>
                
                </div>
            </div>

            <div className="project-item"><img src="/assets/work4.png" alt="projects" />
                <div className="project-content">
                    <h3 className="title">Portfolio</h3>
                    <a href="https://www.ellcrys.org/team" className="">Ugoo.pro </a>
                    <a href="https://www.ellcrys.org/team" className="">Github </a>
                
                </div>
            </div>


                       

        </div>



    );
}
 
export default Works;

